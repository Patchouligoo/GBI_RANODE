...
class PredictBkgProb(
    ...
    def run(self):
        ...
        # load the sample to compare with
        data_trainval_SR_B = np.load(self.input()["preprocessed_data"]["SR_data_trainval_model_B"].path)
        trainvaltensor_SR_B = torch.from_numpy(data_trainval_SR_B.astype('float32')).to(self.device)
        ...
        for model_B in model_Bs:
            with torch.no_grad():
                log_B_trainval = model_B.model.log_probs(inputs=trainvaltensor_SR_B[:,1:-1], cond_inputs=trainvaltensor_SR_B[:,0].reshape(-1,1))
                # set all nans to 0
